...
|`configuration`|HNSWConfiguration|No|Index configuration (dimension and distance metric). If not provided, defaults to `dimension=384`, `distance='cosine'`. If set to `None`, the dimension is derived from `embedding_function`.|HNSWConfiguration(dimension=384, distance='cosine')|
...
|`dimension`|int|No|Vector dimension.|384|
|`embedding_function`|EmbeddingFunction|No|The embedding function used by the collection.|DefaultEmbeddingFunction(model_name='all-MiniLM-L6-v2')|
|`distance`|string|No|Distance metric.|cosine|
|`metadata`|dict|No|Reserved; currently unused.|{}|
