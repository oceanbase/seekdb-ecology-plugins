...
#### 方法 1：使用 PyPI 安装器（推荐，支持多工具）

本包可从 PyPI 安装，并提供支持多种 AI 编程工具的交互式命令行安装器：
...
在安装 Claude Code 后，按以下步骤安装 seekdb 插件：

1. **添加 seekdb 市场**
   
   打开 Claude Code 的终端或命令界面并执行：
   ```bash
   /plugin marketplace add oceanbase/seekdb-ecology-plugins
   ```
   
   该命令会为当前 Claude Code 实例添加 seekdb 插件市场。
...
2. **安装插件**
   ...
   将下载并安装 seekdb 插件及其全部技能。

3. **重启 Claude Code**
   
   安装完成后重启 Claude Code，以确保插件完全加载并可用。

### 配置

插件安装完成后即可使用，无需额外配置。当你提出与 seekdb 相关的问题时，技能会自动对 Claude Code 可用。
...
│
└── src/                                # Python 包源码
    └── seekdb_plugin_installer/        # 包目录
        ├── __init__.py
        ├── main.py                     # CLI 入口（交互式安装器）
        └── skills/                     # 技能（构建时同步）
            └── ...                     # 与根目录 skills/ 结构相同
```
...
本项目以 Python 包形式打包，可发布到 PyPI 便于分发。

**包配置：**
- 包名：`seekdb-agent-skills`
- 入口命令：`seekdb-agent-skills`（在 `pyproject.toml` 中定义）
- 依赖：`questionary>=1.10.0`（用于交互式 CLI）
...
**注意：** 发布新版本前请在 `pyproject.toml` 中更新版本号。
